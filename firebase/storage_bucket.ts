...
export const uploadImage = async (uid: string, file: File) => {
    ...
    return uploadBytes(imageRef, file).then(() => { updateLoadingToastAsSuccess("Image successfully submitted 🔥", "", "img-upload-toast-id") }).catch(() => { updateLoadingToastAsFailure("Image submission failed ❌", "", "img-upload-toast-id") })
};
